...
const QuestionScreen = ({ route }) => {
  ...
  useEffect(() => {
    ...
    const getQuestions = async () => {
      ...
      get(questionsRef).then((snapshot) => {
        if (!snapshot.exists()) {
          console.log(
            "Questions does not exist in database at time",
            Date.now()
          );
          return;
        }
  ...
  useEffect(() => {
    const questionNumberRef = ref(
      db,
      `lobbies/${gameCode}/gameStatus/questionNumber`
    );
  ...
  const handleAnswer = (userId) => {
    ...
    if (currentSong.userId === userId) {
      ...
      const userScoreRef = ref(
        db,
        `lobbies/${gameCode}/users/${user.uid}/score`
      );
  ...
  const handleNextQuestion = () => {
    ...
    runTransaction(gameStatusRef, (status) => {
      if (
        status &&
        status.questionNumber !== undefined &&
    ...
    <View style={[styles.container]}>
      ...
      {currentSong ? (
        <>
          <Text style={styles.subtitle}>
            Pick the user who likes this song:
          </Text>
          ...
          <View style={styles.highLowContainer}>
            ...
            <Text style={styles.highLowText}>
              {`Song: ${currentSong.name} by ${currentSong.artists}`}
            </Text>
          </View>
          ...
          {users.map((user) => (
            ...
            >
              <Text style={styles.gameText}>{user.username}</Text>
            </Pressable>
          ))}
        </>
      ) : (
      ...
      {hostUID === user.uid && (
        <Pressable
          ...
          style={styles.rouletteButton}
        >
          <Text style={styles.gameText}>Next Question</Text>
        </Pressable>
      )}
    </View>
  );
